...
def refineVertices(vertices, sdf, iterations=5):
    for _ in range(iterations):
        x, y, z = [vertices[:, i] for i in range(3)]
        grad = sdf((x, y, z)).T
        value = get_gradient(sdf, (x, y, z))
        vertices = (
            vertices - grad * ((value / np.sum(grad**2, axis=1)))[:, np.newaxis]
        )
        ...
        for j in range(N):
            for k in range(N):
                ...
                if not (np.min(z) > level or np.max(z) < level):
                    ...
                    # verts = refineVertices(verts, sdf, iterations=5)
